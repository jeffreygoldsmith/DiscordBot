...
handlers.slow = (message, author) =>
{
  ...
  if (channelFound)
  {
    ...
    const path = '../sounds/' + slowpokeSoundNames[randomNum]; // Choose audio file to play and create a path string for it

    // Enter the channel that the user is currently in
    channelFound.join().then(connection => {
      const dispatcher = connection.playStream(path); // Play the file
